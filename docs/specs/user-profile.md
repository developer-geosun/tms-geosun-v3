...
- [ ] Користувач зберігає ПІБ, тип особи, ЄДРПОУ (для юрособи), телефони з месенджерами і канали зв’язку.
- [ ] `INDIVIDUAL` не приймає ЄДРПОУ; представник юрособи не зберігається без валідного ЄДРПОУ.
- [ ] Канал `PHONE` вимагає ≥1 телефон; `MESSENGERS` — ≥1 месенджер-прапорець.
- [ ] `GET /auth/me` і login повертають `displayName` + `profile`.
- [ ] `/profile` працює на desktop і handset, i18n ua/en/ru.
- [ ] ADMIN бачить і редагує профіль у `/admin/users`; MANAGER лише читає.
- [ ] Незаповнений профіль не блокує маршрути/заявки; `profileComplete` коректний.
- [ ] Documentation updated / Документация обновлена (`system.md`, статус цієї спеки).
- [ ] Tests added/updated and passing / Тесты добавлены или обновлены и проходят.
- [ ] Схема таблиці `users` не змінена (немає ALTER).
