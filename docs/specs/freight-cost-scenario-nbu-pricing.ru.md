...
- **Клиент:** Angular + Java
- **Baseline:** [BASELINE.ru.md](BASELINE.ru.md) **B08**; формулы чисел — **B09** (при расхождении приоритет у файла формул)
- **Остаток (бэклог, v1 не блокируют):** опционально — PATCH сценариев/тарифов (есть PUT); `nameUk` в таблице breakdown на заявке. Эталонный unit-тест сумм — вне v1. Детали чекбоксов — §12
...
- **Правила расчёта (формулы, статьи затрат, v1):** [`freight-trip-cost-calculation-rules-margin30-ua8150-driverpct.ru.md`](./freight-trip-cost-calculation-rules-margin30-ua8150-driverpct.ru.md) — **источник истины** для детерминированного калькулятора; при расхождении с этим ТЗ приоритет у файла правил.
- Связанные документы: `currencies-reference.ru.md`, `routes-server-workflow-and-freight-quoting.ru.en.md`, `route-immutability-list-filters-deferred-country-breakdown.ru.md`, `route-point-operations-rules.ru.md`, `auth-authentication-authorization.ru.en.md`, `system.ru.md`.
...
- формулы — по [файлу правил](./freight-trip-cost-calculation-rules-margin30-ua8150-driverpct.ru.md).
...
- Курсы НБУ: `currency_nbu_rates`, sync/чтение — `currencies-reference.ru.md`; **не дублировать** API НБУ в расчёте фрахта.
...
- [x] 2. **Расчёт себестоимости** по [правилам](./freight-trip-cost-calculation-rules-margin30-ua8150-driverpct.ru.md), JSON **breakdown**, аудит входов.
...
- `POST /api/v1/route-requests` **не** запускает breakdown (`route-immutability-list-filters-deferred-country-breakdown.ru.md` §3.3).
...
Серверная сущность: `id`, `name`, `isActive`, аудит. Числовые поля **задаёт ADMIN/MANAGER**; эталон v1 — в [правилах](./freight-trip-cost-calculation-rules-margin30-ua8150-driverpct.ru.md).
...
- Seed v1: один набор «EU default» со ставками из [правил](./freight-trip-cost-calculation-rules-margin30-ua8150-driverpct.ru.md); дефолтный сценарий ссылается на него.
...
## 6. Курсы НБУ
- Источник: `currency_nbu_rates`; sync — `POST /api/v1/admin/currencies/nbu-rates/sync` (`currencies-reference.ru.md`).
...
## 7. Данные и аудит
- **`FreightCostCalculation`** / **`FreightQuote`**: `route_request_id`, `scenario_id`, `calculation_date`, снимок сценария, JSON breakdown, длины, `driver_salary_basis` — перечень полей breakdown см. [правила](./freight-trip-cost-calculation-rules-margin30-ua8150-driverpct.ru.md) § «Аудит».
...
- [x] Реализация формул v1 — по [файлу правил](./freight-trip-cost-calculation-rules-margin30-ua8150-driverpct.ru.md) (код калькулятора; **автотесты эталона — нет**).
...
| # | Тема | Решение |
|---|------|---------|
| 11.1 | Формулы и дефолты v1 | [`freight-trip-cost-calculation-rules-margin30-ua8150-driverpct.ru.md`](./freight-trip-cost-calculation-rules-margin30-ua8150-driverpct.ru.md) |
