...
- **Роль:** источник истины по будущему OCR/suggest
- **Клиент:** Angular + Java
- **Остаток:** весь scope. Целевой этап — после v2 интеграции `document_types` с карточками ТС/водителей
...
## Связанные документы
- [`document-types-reference.ru.md`](document-types-reference.ru.md) — источник `field_definitions`, `planned_scan_pages`, `country_code`.
- [`file-storage.ru.md`](file-storage.ru.md) — `StoredFileService`, лимит 10 MB, MIME.
- [`drivers-and-vehicle-combinations.ru.en.md`](drivers-and-vehicle-combinations.ru.en.md) — `driver_documents` (`side` = FRONT/BACK), сканы прав/паспортов.
- [`vehicles-reference.ru.md`](vehicles-reference.ru.md) — `vehicle_documents`, свидетельство о регистрации.
- [`auth-authentication-authorization.ru.en.md`](auth-authentication-authorization.ru.en.md) — RBAC, `ApiException`, коды ошибок.
