...
- **Related docs / Связанные документы:**
  - [`auth-authentication-authorization.ru.en.md`](auth-authentication-authorization.ru.en.md)
  - [`admin-user-management.ru.en.md`](admin-user-management.ru.en.md)
  - [`drivers-and-vehicle-combinations.ru.en.md`](drivers-and-vehicle-combinations.ru.en.md) — правила ПІБ, отдельная сущность
  - [`chatbots-telegram-whatsapp-viber.ru.en.md`](chatbots-telegram-whatsapp-viber.ru.en.md) — identity бота (`bot_identities`), не колонки профиля; `@username` по-прежнему вне профиля
  - [`admin-notify-new-user-registration.ru.en.md`](admin-notify-new-user-registration.ru.en.md) — каналы профиля ADMIN используются и как транспорт системного алерта «новая регистрация»
...
- Верификация телефонов через **SMS OTP** (вне scope). Верификация телефона через **Telegram** (share contact) — см. [`chatbots-telegram-whatsapp-viber.ru.en.md`](chatbots-telegram-whatsapp-viber.ru.en.md).
- Аватар / фото.
...
12a. Для ролі **ADMIN** ті самі прапорці — транспорт системного сповіщення «нова реєстрація» ([admin-notify-new-user-registration.md](admin-notify-new-user-registration.ru.en.md)). Правила обов’язковості каналів у формі профілю **не** змінюються цим ТЗ.
...
5. Оновити `docs/system.ru.md` і цей файл (статус «реалізовано»).
...
- [x] Documentation updated / Документация обновлена (`system.ru.md`, статус цієї спеки).
