...
- **Related docs / Связанные документы:**
  - `docs/specs/auth-authentication-authorization.ru.en.md` (**основной источник истины по auth и ролям**).
  - `docs/specs/route-point-operations-rules.ru.md` (**canonical source для правил операций точек маршрута**).
...
- Расчет протяженности маршрута по странам на backend **по явному шагу** в админском/менеджерском контуре (см. `docs/specs/route-immutability-list-filters-deferred-country-breakdown.ru.md`); **не** при создании пользовательской заявки `POST /api/v1/route-requests`. Расчёт фрахта — см. `docs/specs/freight-cost-scenario-nbu-pricing.ru.md`.
...
7. Backend рассчитывает и сохраняет breakdown расстояния по странам **после** явного запроса расчёта (отдельный admin endpoint); при **`POST /api/v1/route-requests`** расчёт и сохранение breakdown **не** выполняются. Расчёт коммерческой ставки — см. `docs/specs/freight-cost-scenario-nbu-pricing.ru.md`.
8. После создания заявки по `routeId` изменение snapshot этого маршрута запрещено; для правок пользователь дублирует маршрут (новый `id`) — см. `docs/specs/route-immutability-list-filters-deferred-country-breakdown.ru.md`. Повторные пересчёты геометрии для того же snapshot при привязанной заявке не допускаются.
...
- **Security / Безопасность:**
  - Следовать требованиям `docs/specs/auth-authentication-authorization.ru.en.md` для `401/403`, bearer, RBAC.
...
> Все endpoint-ы ниже защищены `Bearer access token` по правилам `docs/specs/auth-authentication-authorization.ru.en.md`.
...
- `GET /api/v1/routes/my` - список маршрутов текущего пользователя (параметр `view` — см. `docs/specs/route-immutability-list-filters-deferred-country-breakdown.ru.md` §3.2).
...
- Обязательно переиспользовать текущий auth middleware и RBAC политику из `docs/specs/auth-authentication-authorization.ru.en.md`.
...
- Любые изменения правил операций точек маршрута выполнять с обязательной синхронизацией с `docs/specs/route-point-operations-rules.ru.md`.
...
- При расхождении между старыми описаниями в release notes и этим ТЗ приоритет имеет данная спецификация и auth-спецификация `docs/specs/auth-authentication-authorization.ru.en.md`.
...
7. Обновить `docs/system.ru.md` и release notes.

## 13) Acceptance Criteria (Definition of Done) / Критерии приемки
- [x] Пользователь может сохранить маршрут и увидеть его в своей истории.
- [x] Пользователь может открыть любой свой сохраненный маршрут.
- [x] Пользователь не может открыть/изменить маршрут другого пользователя (`403/404` по политике безопасности).
- [x] Пользователь может отправить запрос на фрахт по сохраненному маршруту.
- [x] Admin видит список отправленных запросов и может открыть детали маршрута.
- [x] Admin может создать и отправить `quote`; история предложений доступна.
- [x] После явного шага расчёта (admin/manager) для заявки сохранён breakdown расстояния по странам; до этого шага создание user-заявки не создаёт breakdown.
- [x] Все проверки доступа соответствуют auth-спецификации.
- [x] Добавлены и проходят unit/integration тесты для критичных сценариев.
- [x] Документация обновлена (включая ссылку на auth-спецификацию).
...
- **Assumptions / Допущения:**
  - Auth-контур из `docs/specs/auth-authentication-authorization.ru.en.md` внедрен и стабилен.
...
3. Проверки `auth`/`RBAC` реализовывать только в соответствии с `docs/specs/auth-authentication-authorization.ru.en.md`.
