...
- `role`: в MVP при `POST /auth/register` **не передается клиентом**; сервер назначает `USER`. Смена ролей и admin-управление пользователями — см. [`admin-user-management.ru.en.md`](admin-user-management.ru.en.md)
