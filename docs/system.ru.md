...
- Деплой frontend на GitHub Pages через GitHub Actions (`main`/`master`, source **GitHub Actions**); публичный API — по выбору через ngrok или статический IP провайдера (только backend, см. `PUBLIC_ACCESS_MODE` в `.env` / `RUN.ru.md`).
...
- Управление ролями / active / soft-delete — только `ADMIN` (см. `docs/specs/admin-user-management.ru.en.md`).
- Контактный профиль учётки: self GET/PUT для любой роли; чужой GET — `ADMIN`/`MANAGER`; чужой PUT — `ADMIN` (см. `docs/specs/user-profile.ru.en.md`).
