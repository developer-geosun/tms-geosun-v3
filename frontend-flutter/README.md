...
`API_URL` для Flutter берётся из GitHub Secret `API_URL` (тот же, что для Angular). Deep links на Pages обрабатываются через общий `404.html` в корне сайта (файл `.github/pages-404.html` копируется в артефакт при деплое). Маршруты — path URL (`/flutter/reset-password?token=`), не hash (`#/login`). Ссылки из писем ведут на клиент, который отправил запрос (`X-App-Client`).
